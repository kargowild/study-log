...
프로세스는 각자 독립적인 메모리 공간을 할당받는데, 한 프로세스의 여러 스레드는 여기서 코드, 데이터, 힙 영역을 공유하고, 
스택 영역만 독립적으로 사용함으로써 메모리를 효율적으로 사용할 수 있습니다.
메모리를 공유하는 덕분에 스레드간 컨텍스트 스위칭에 드는 비용은 프로세스간 컨텍스트 스위칭에 드는 비용보다 작습니다.
또한 프로세스간 통신이 필요한 경우, OS의 도움을 받아 IPC를 해야하지만, 스레드 간 통신이 필요한 경우에는
메모리 공간을 서로 공유하기 때문에 시스템콜이 필요 없으나 그만큼 동시성 문제를 신경써야 합니다.

### 꼬리질문. 프로세스간 컨텍스트 스위칭 비용보다 스레드간 컨텍스트 스위칭 비용이 작은 이유가 무엇인가요?
한 프로세스의 여러 스레드끼리는 스택을 제외한 메모리 공간을 공유하기 때문에, 
컨텍스트스위칭이 일어나더라도 페이지 테이블과 TLB를 그대로 사용할 수 있습니다.
따라서 각종 레지스터 값만 문맥에 맞게 교체해주면 되기 때문에 스레드간 컨텍스트스위칭 비용은 비교적 작습니다. 
...
하나의 프로세스에 속한 여러 스레드는 스택 메모리만 독립적으로 사용하고, 그 외의 메모리 공간을 공유하기 때문에 자원을 효율적으로 사용할 수 있습니다.
컨텍스트 스위칭 과정에서도 멀티 스레드의 경우 페이지 테이블과 TLB를 공유하므로 비용이 비교적 작습니다. 
...
### 꼬리질문. 서버 애플리케이션에서 멀티 스레드를 사용하는 이유는?
서버 애플리케이션이 싱글스레드 프로세스라면, DB 접근과 같이 IO 작업을 할 때 스레드가 블로킹되면서 CPU가 일하지 않고 놀게 됩니다.
멀티 스레드 프로세스라면, 이 때 다른 사용자 요청에 할당된 스레드에게 CPU를 할당함으로써 CPU utilization을 높일 수 있습니다.
...
### 꼬리질문. 그래프에서 사이클이 존재하는지 판단하려면 어떤 알고리즘을 사용해야 하나요?
모든 노드를 방문해가면서 탐색해봐야 하므로 DFS 또는 BFS 알고리즘을 사용할 수 있습니다.
-> 직접 짜보기.  
