    ...
    - 가변분할 방식
      ...
      - 주소 공간의 크기가 n인 프로세스를 동적메모리 할당하기 위한 방법
        ...
        - 최악적합 (제일 구림)
          ...
          - 상대적으로 더 큰 프로그램을 담을 수 있는 가용 공간을 빨리 소진한다는 문저젬 존재
  ...
  - 불연속할당
    ...
    - 페이징 기법
      ...
      - 페이지 테이블은 주소 변환을 하기 위한 자료구조로, 물리적 메모리에 위치한다.
        - 운영체제는 현재 CPU에서 실행 중인 프로세스의 페이지 테이블에 접근하기 위해 페이지 테이블 기준 레지스터와 페이지 테이블 길이 레지스터를 참고한다.
      ...
      - 역페이지 테이블
        ...
        - 페이지 테이블의 각 항목은 어느 프로세스의 어느 페이지가 이 프레임에 저장되었는지의 정보를 보관하고 있다.
          ...
          - 페지이 테이블 전체를 다 탐색해야 한다. -> TLB와 같이 연관 레지스터에 저장해서 병렬적으로 탐색하자.
    - 세그먼테이션 기법
