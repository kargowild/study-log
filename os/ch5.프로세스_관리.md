...
- CPU는 프로세스 B를 실행 중에 인터럽트를 확인하고 운영체제가 프로세스 B로부터 CPU를 뺏어와 인터럽트 처리 루틴을 수행한다.
...
- 프로세스 생성 절차 유닉스 예
  ...
  - 이 때 프로세스 ID를 제외한 모든 정보(운영체제 커널 내의 정보와 주소 공간의 정보)를 그대로 복사
    ...
    - 자식 프로세스에게 부모와는 다른 독자적인 프로그램을 수행시킬 수 있는 메커니즘이 필요하다. -> exec()!
...
- 프로세스의 종료
  ...
  - 비자발적 종료
    - 부모 프로세스가 abort() 함수를 사용해 자식 프로세스의 수행을 강제로 종료
      ...
      - 부모 프로세스가 종료되는 경우
  - 부모 프로세스가 종료될 때 자식 프로세스를 계속 실행시키기 위해서 종료되지 않을 다른 프로세스의 양자로 자식 프로세스를 보내는 방식도 있다.
...
- IPC의 대표적인 방법
  - 메시지 전달 방식
    ...
    - 메시지 전송 대상에 따른 분류
      - 직접통신
        - 전송 대상이 다른 프로세스
        - 커뮤니케이션 링크는 자동으로 생성되고, 하나의 링크는 정확히 한 쌍의 프로세스에게 할당된다.
  ...
  - 공유 메모리 방식
    ...
    - 데이터 일관성 문제가 발생할 수 있는데, 커널이 책임지지 않기 때문에 프로세스들끼리 직접 공유메모리 접근에 대한 동기화 문제를 책임져야 한다.
