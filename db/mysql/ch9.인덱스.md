...
- 쿼리 실행 계획을 보면 보조 인덱스에는 Key Lookup이라는 말이 있다.
  ...
  - 예를 들어 BBK로 검색할 경우 인덱스 페이지에서 검색한 RID인 '1002+#2'의 데이터 페이지로 찾아가는 과정이라고 생각하면 쉽게 이해된다.
...
- WHERE 절에 사용되더라도 자주 사용해야 가치가 있다.
  ...
  - 심지어 클러스터형 인덱스라면 데이터가 입력되는 과정에서 발생하는 페이지 분할이 꽤나 부담스럽다.
    - 이게 PK는 auto increment 하는 대체키를 사용하는 근거 중 하나가 될 수도 있겠다. 자연키를 어떻게든 사용하면 그게 항상 리프페이지 맨 끝에 오는 보장이 없으니
