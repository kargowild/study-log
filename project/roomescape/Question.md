...
OOP의 핵심 원칙으로는 캡슐화, 상속, 다형성, 추상화가 있습니다. (일단 여기까지) 
...
다형성은 인터페이스와 같은 부모 타입으로 여러 자식 타입을 받음으로써 같은 메서드 시그니쳐를 호출해도 각자 오버라이딩한 기능을 수행하므로
추상화와 함께 사용했을 때 좋은 시너지를 낼 수 있습니다. 예를 들어 의존관계 주입도 다형성과 추상화를 활용한 방법입니다.

### 꼬리질문3. SOLID에 대해 설명해주세요.
S는 단일책임원칙으로 하나의 객체가 변경되어야 하는 이유가 단 하나여야 한다는 원칙이며, 이를 통해 유지보수성을 높일 수 있다는 원칙입니다.
...
D는 의존관계 역전 원칙으로, 객체간 의존시 구현 클래스보다 추상클래스나 인터페이스에 의존하도록 함으로써
...
또한 DB 접근을 위해 JdbcTemplate을 제공한다거나, JPA와 같은 ORM의 도움을 쉽게 받을 수 있다는 점에서도 매력을 느꼈고
...
외부 API를 호출해서 받은 응답을 DTO로 변환하고 있다면, 목 서버가 API 스펙에 맞게 응답을 내려줬을 때, DTO 변환이 잘 일어나는지 확인해야 합니다.
...
특정 컬럼의 값이 테이블 내에서 유일해야 하는 조건이며 대표적으로 PK에는 Unique 제약조건이 기본적으로 포함되어있습니다.
